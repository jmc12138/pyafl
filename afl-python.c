...
int __parse_args(const char *json_str){
    ...
    if ((item = cJSON_GetObjectItem(root, "exec_tmout")) != NULL) {
        u8 suffix = 0;
        if (sscanf(item->valuestring, "%u%c", &exec_tmout, &suffix) < 1 ||
            item->valuestring[0] == '-') FATAL("Bad syntax used for exec_tmout");
        if (exec_tmout < 5) FATAL("Dangerously low value of exec_tmout");
        if (suffix == '+') timeout_given = 2; else timeout_given = 1;
        if (verbose) OKF("Execution timeout: %u ms (%s)", 
                        exec_tmout, 
                        timeout_given == 2 ? "soft limit" : "hard limit");
    }

    /* 内存限制 */
    if ((item = cJSON_GetObjectItem(root, "mem_limit")) != NULL) {
        if (!strcmp(item->valuestring, "none")) {
            mem_limit = 0;
            if (verbose) OKF("Memory limit: none");
        } else {
            u8 suffix = 'M';
            mem_limit_given = 1;
            if (sscanf(item->valuestring, "%llu%c", &mem_limit, &suffix) < 1 ||
                item->valuestring[0] == '-') FATAL("Bad syntax used for mem_limit");
            switch (suffix) {
                case 'T': mem_limit *= 1024 * 1024; break;
                case 'G': mem_limit *= 1024; break;
                case 'k': mem_limit /= 1024; break;
                case 'M': break;
                default:  FATAL("Unsupported suffix or bad syntax for mem_limit");
            }
            if (mem_limit < 5) FATAL("Dangerously low value of mem_limit");
            if (sizeof(rlim_t) == 4 && mem_limit > 2000)
                FATAL("Value of mem_limit out of range on 32-bit systems");
            if (verbose) OKF("Memory limit: %llu MB", mem_limit);
        }
    }

    /* 跳过确定性阶段 */
    if ((item = cJSON_GetObjectItem(root, "skip_deterministic")) != NULL) {
        if (!strcasecmp(item->valuestring, "True")) {
            skip_deterministic = 1;
            use_splicing = 1;
            if (verbose) OKF("Skip deterministic: enabled");
        } else if (verbose) {
            OKF("Skip deterministic: disabled");
        }
    }

    /* 崩溃模式 */
    if ((item = cJSON_GetObjectItem(root, "crash_mode")) != NULL) {
        if (!strcasecmp(item->valuestring, "True")) {
            crash_mode = FAULT_CRASH;
            if (verbose) OKF("Crash mode: enabled");
        } else if (verbose) {
            OKF("Crash mode: disabled");
        }
    }

    /* dumb模式 */
    if ((item = cJSON_GetObjectItem(root, "dumb_mode")) != NULL) {
        if (!strcasecmp(item->valuestring, "True")) {
            if (getenv("AFL_DUMB_FORKSRV")) dumb_mode = 2; else dumb_mode = 1;  
            if (verbose) OKF("Dumb mode: %d", dumb_mode);
        } else if (verbose) {
            OKF("Dumb mode: disabled");
        }
    }

    /* 网络配置 */
    if ((item = cJSON_GetObjectItem(root, "use_net")) != NULL) {
        if (parse_net_config(item->valuestring, &net_protocol, &net_ip, &net_port))
            FATAL("Bad syntax used for use_net. Check the network setting. [tcp/udp]://127.0.0.1/port");
        use_net = 1;
        if (verbose) OKF("Network: %s://%s/%u", net_protocol, net_ip, net_port);
    }
    ...
    /* 终止子进程 */
    if ((item = cJSON_GetObjectItem(root, "terminate_child")) != NULL) {
        if (!strcasecmp(item->valuestring, "True")) {
            terminate_child = 1;
            if (verbose) OKF("Terminate child: enabled");
        } else if (verbose) {
            OKF("Terminate child: disabled");
        }
    }
    ...
    if ((item = cJSON_GetObjectItem(root, "target_cmd")) != NULL) {


        // 第一次遍历计算需要多少空间
        char *token = strtok(item->valuestring, " ");

        int i = 0;
        while (token != NULL) {
            target_cmd[i] = strdup(token); // 动态分配并复制字符串
            if (!target_cmd[i]) FATAL("内存分配失败");
            
            if (verbose) OKF("   Command part %d: %s", i, target_cmd[i]);
            
            token = strtok(NULL, " ");
            i++;
        }
        
        target_cmd[i] = NULL; // NULL终止数组
        target_cmd_len = i;   // 存储实际长度
        
        if (i == 0) {  // 没有空格的情况
            target_cmd[0] = strdup(item->valuestring);
            if (!target_cmd[0]) FATAL("内存分配失败");
            target_cmd[1] = NULL; // NULL终止
            target_cmd_len = 1;
            if (verbose) OKF("Target command: %s", target_cmd[0]);
        }
    }
    /* 名称 */
    if ((item = cJSON_GetObjectItem(root, "name")) != NULL) {
        strncpy(name, item->valuestring, MAX_STR_LEN - 1);
        name[MAX_STR_LEN - 1] = '\0';
        if (verbose) OKF("Session name: %s", name);
    }   



    /* 检查必要参数 */
    if (!use_net) FATAL("Please specify network information of the server under test");




    if (verbose) {
        SAYF(cCYA "== End of Configuration ==\n\n" cRST);
    }
...
void __clear(){
  if (target_cmd) {
  for (int i = 0; target_cmd[i] != NULL; i++) {
      free(target_cmd[i]);
  }
...
void __get_test_case(const char *buf, size_t buf_len) {
    ...
    // 2. 智能内存调整（realloc优化）
    char *new_buf = realloc(global_buf, buf_len);  // 关键优化点
    if (new_buf == NULL) {
        // 失败时保留原数据，确保程序可继续运行
        FATAL("Failed to (re)allocate %zu bytes", buf_len);
    }

    // 3. 更新指针和长度
    global_buf = new_buf;
    global_buf_len = buf_len;
